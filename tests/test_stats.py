...
class StatsHelpersTest(unittest.TestCase):
    ...
    def test_prepare_domain_frame_replaces_partial_vader_columns(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "domain.csv"
            pd.DataFrame(
                {
            ...
            prepared = prepare_domain_frame(path)

            self.assertEqual(list(prepared.filter(regex=r"^vader_").columns), [
                "vader_neg",
                "vader_neu",
                "vader_pos",
                "vader_compound",
            ])
            self.assertEqual(prepared.filter(regex=r"^vader_").shape[1], 4)
